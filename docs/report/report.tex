...
%
% TODO before submitting: put the second group member's name and e-mail in \author.
...
\and
\IEEEauthorblockN{Second Group Member}
\IEEEauthorblockA{Sapienza University of Rome\\
name.matricola@studenti.uniroma1.it}
}
...
result must remain interpretable when the subset gets small. We use
t-SNE~\cite{tsne} for the projection and report its trustworthiness~\cite{venna} with
every local re-projection, and we follow the warnings collected by Wattenberg et
al.~\cite{misread} about how t-SNE pictures are misread: axes without units, cluster
sizes that do not mean density, and inter-cluster distances that do not mean
dissimilarity. Those warnings drove concrete design decisions in Section~\ref{sec:design}.
...
layout: an equal-area projection made Europe --- where the corpus is densest and the
countries are smallest --- hard to read and to click, while Mercator stretched the north
so fast that a dashboard-shaped card had to crop Scandinavia. Miller is neither conformal
nor equal-area, and the report states so rather than hiding it.
...
against 40\%. The projection separated what the operation does and how deep it gets, which
is the cut a defender needs and one that no actor-based grouping would produce.
...
The suites have repeatedly earned their keep. They caught a CSS rule that silently
overrode the fading of unreliable bars; a plain click in the projection that re-projected
a country selection; and, most recently, a redraw-on-resize implementation that reached
